...
**Primary (shipped):** [`client/js/calm-viz.js`](../../client/js/calm-viz.js) — procedural canvas modes `neural`, `tissue`, `mandala` on `#viz`. While audio plays, visuals react in real time to the master-bus analyser via `CalmMind.frame`; on the compose screen, motion is idle/synthetic (not FFT-driven). See [`subsystems/visualization.md`](../../subsystems/visualization.md).

**Legacy:** [`client/js/visualizer.js`](../../client/js/visualizer.js) on `#visualizerCanvas` (`index-legacy.html` only).

An alternate modular Three.js visualizer (`ThreeDVisualizer`) is archived under [`archive/src-modular-unwired/visualization/`](../../archive/src-modular-unwired/visualization/).
